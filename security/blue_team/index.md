...
| **🟡 L2** | **2.1 邊界防火牆策略與微創阻斷實務**<br>(iptables In-line 串接、DROP 規則與連線追蹤) | • `07_藍隊防禦與護網營運/02_存取控制與防火牆/Linux存取控制與防火牆/`<br>• [Netfilter iptables 官方封包過濾與狀態追蹤手冊](https://netfilter.org/)<br>• [`final_mock_exams/mock_exam_b_lab_questions.md`](final_mock_exams/mock_exam_b_lab_questions.md)（第 24~28 題） |
...
| **👑 L4** | **5.3 容器逃逸與 K8s 叢集運行時安全**<br>(Kubernetes Goat 演練、Privileged 容器逃逸) | • [madhuakula: Kubernetes Goat (K8s 容器攻防與日誌偵測)](https://github.com/madhuakula/kubernetes-goat)<br>• [CyberDefenders: ContainerBreak (容器逃逸調查)](https://cyberdefenders.org/blueteam-ctf-challenges/containerbreak-rootkit-trail/)<br>• [Kube-bench CIS Kubernetes Benchmark 自動化審計](https://github.com/aquasecurity/kube-bench) |
...
| **🟢 L1** | **6.3 DNS 基礎查詢解析與異常頻率排查**<br>(A/AAAA/TXT/PTR 查詢特徵、NXDOMAIN 洪泛) | • [CyberDefenders: PacketDetective (DNS 封包排查專題)](https://cyberdefenders.org/blueteam-ctf-challenges/packetdetective/)<br>• [MTA: 2021-02-01 Emotet 異常 DNS 查詢樣本分析](https://www.malware-traffic-analysis.net/2021/02/01/index.html) |
...
| **🔴 L3** | **12.3 偽造票據攻擊與全域特權維持**<br>(Golden Ticket 偽造 TGT / Silver Ticket 偽造 ST) | • [`final_mock_exams/mock_exam_a_100q_questions.md`](final_mock_exams/mock_exam_a_100q_questions.md)（Kerberos 核心題群）<br>• [BloodHound 官方開源專案 (AD 攻擊路徑繪製)](https://github.com/SpecterOps/BloodHound) |
...
| **🟡 L2** | **16.1 DNS 隱蔽通道與外洩特徵鑑識**<br>(DNS Tunneling, dnscat2, 異常長子網域) | • [CyberDefenders: PacketDetective (PCAP 鑑識)](https://cyberdefenders.org/blueteam-ctf-challenges/packetdetective/)<br>• [MTA: 2020-09-02 DNS Tunneling 流量樣本分析](https://www.malware-traffic-analysis.net/)<br>• [`final_mock_exams/mock_exam_b_lab_questions.md`](final_mock_exams/mock_exam_b_lab_questions.md)（第 30~35 題：DNS 隧道推演） |
...
| **🔴 L3** | **20.2 加密 WebShell 流量辨析與日誌還原**<br>(冰蠍 Behinder / 哥斯拉 Godzilla 流量解密) | • [PortSwigger: File Upload Attacks (Webshell 上傳防禦實驗)](https://portswigger.net/web-security/file-upload)<br>• [`final_mock_exams/mock_exam_b_lab_questions.md`](final_mock_exams/mock_exam_b_lab_questions.md)（第 15~23 題） |
...
| **🟡 L2** | **24.1 NTFS 主檔案表與時間戳偽造辨析**<br>(`$MFT` / `$STANDARD_INFORMATION` vs `$FILE_NAME`) | • [HTB Sherlocks: BFT (MFT 主檔案表與 Timestomping 深度鑑識)](https://app.hackthebox.com/sherlocks)<br>• [Eric Zimmerman's MFTECmd 實機解析演練](https://ericzimmerman.github.io/)<br>• [`final_mock_exams/mock_exam_b_lab_questions.md`](final_mock_exams/mock_exam_b_lab_questions.md)（第 12~14 題） |
...
| **🔴 L3** | **25.1 Linux 核心模組 Rootkit 與隱蔽技術**<br>(LKM Hooking, `LD_PRELOAD` 動態庫劫持) | • [CyberDefenders: ContainerBreak (Linux 容器逃逸與 Rootkit)](https://cyberdefenders.org/blueteam-ctf-challenges/containerbreak-rootkit-trail/)<br>• [chkrootkit / rkhunter (Linux 核心模組檢測)](http://www.chkrootkit.org/) |
...
| **👑 L4** | **28.4 台灣資安法規與實體攻防檢定真題演練**<br>(台灣資通安全事件通報、實體推演題本與答案解構) | • [`final_mock_exams/mock_exam_b_lab_questions.md`](final_mock_exams/mock_exam_b_lab_questions.md)（全套 81 題實體推演）<br>• [`final_mock_exams/mock_exam_b_lab_solutions.md`](final_mock_exams/mock_exam_b_lab_solutions.md)<br>• [`final_mock_exams/mock_exam_a_100q_questions.md`](final_mock_exams/mock_exam_a_100q_questions.md) |
...
| 難度 | 細分實戰技術點 (Sub-Topic & Technique) | 🎯 具體線上靶場、實戰房間、開源數據集與題目清單 (Practicable Challenges & Labs) |
| :---: | :--- | :--- |
| **🟢 L1** | **29.1 數位證據監管鏈與 ISO/IEC 27037**<br>(證據識別、收集、獲取與保存標準程序) | • [NIST SP 800-86: 數位鑑識整合指引手冊](https://csrc.nist.gov/publications/detail/sp/800-86/final)<br>• [DFRWS 數位證據監管鏈標準指引範本](https://dfrws.org/)<br>• [`final_mock_exams/mock_exam_a_100q_questions.md`](final_mock_exams/mock_exam_a_100q_questions.md)（鑑識法規題群） |
| **🟢 L1** | **29.2 資安事件通報時限與 RFC 3227**<br>(1~4 級事件 1 小時通報、數據揮發次序) | • [國家資通安全研究院: 資安事件分級指引](https://www.nics.nat.gov.tw/)<br>• [SANS Incident Handler's Handbook 應變步驟](https://www.sans.org/white-papers/)<br>• [`final_mock_exams/high_frequency_flashcards.md`](final_mock_exams/high_frequency_flashcards.md) |
