...
> [!IMPORTANT]
> **適用定位**：本手冊為 [`B4_GOLDEN_SHIELD_30DAYS_SPRINT_ROADMAP.md`](B4_GOLDEN_SHIELD_30DAYS_SPRINT_ROADMAP.md) 最終驗收、全真模擬測驗與決賽備戰之核心解析手冊。
> **題源總量統計**：
> 1. **客觀選擇題庫**：本機題庫合計 **1,623 道試題**（`安全题库1~4.xlsx` 共 1,361 題單選/多選題 ＋ 7 份資安筆試題庫 262 題）。
...
4. [🚩 模組三：紅隊滲透 CTF I（30 把 Flag 內網網域奪旗全解）](#-模組三紅隊滲透-ctf-i30-把-flag-內網網域奪旗全解)
5. [⚔️ 模組四：逆向與進階 Web CTF II（6 大題 12 把 Flag 裁判題本）](#-模組四逆向與進階-web-ctf-ii6-大題-12-把-flag-裁判題本)
...
| :--- | :--- | :---: | :--- | :---: |
| **客觀筆試題庫** | `01_Web安全/15_筆試與面試題庫/` | **1,623 題** | 資安法規、網路協定、系統防護、OWASP Web、密碼學 | **初賽 100 題單選** |
| **數位鑑識 (IR)** | `20260728 Skills Competition/題目/IR/` | **23 題** | PCAP 流量分析、JVM 記憶體、Volatility 3、Ext4 磁碟時間戳 | **初賽概念 / 決賽** |
| **主機強化 (Hardening)** | `20260728 Skills Competition/題目/Hardening/` | **28 項** | AD dMSA 遷移、ADCS ESC1/15 緩解、Suricata 規則、Rsyslog TLS | **決賽防禦加固** |
| **紅隊滲透 (CTF I)** | `20260728 Skills Competition/題目/CTFI/` | **30 Flags** | LFI/RCE、Linux 機群橫向、MSSQL 提權、GPP/gMSA、ADCS、Golden Ticket | **初賽概念 / 決賽攻防** |
| **逆向與Web (CTF II)** | `20260728 Skills Competition/題目/CTFII/` | **12 Flags** | Redis 提權、Jolokia WAF 繞過、Golang SSTI、2038 Overflow、GBA 逆向 | **初賽概念 / 決賽攻防** |

---

## 🔍 模組一：數位鑑識 Incident Response 全部 23 題真題解析

> 📂 **原始題本對應**：`20260728 Skills Competition/題目/IR/題目卷_答案卷.md.docx`

### 🌐 題組 1：網路流量分析 (Network Traffic Forensics)

> 📁 **本機現成試題檔案路徑**（大會原始題檔，**絕不需要自己編造**）：  
...
> 📂 **原始題本對應**：`20260728 Skills Competition/題目/Hardening/全國賽 2026 題目設計與答案稿.docx`
...
> 📂 **原始題本對應**：`20260728 Skills Competition/題目/CTFI/`  
...
> 📂 **原始題本對應**：`20260728 Skills Competition/題目/CTFII/題目本.md`
...
> 📂 **來源題庫精萃**：萃取自本機 `安全题库1.xlsx` (895 題)、`安全题库2.xlsx` (407 題) 與資安筆試題庫。
