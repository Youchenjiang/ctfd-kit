...
from flask import Blueprint, Response, request, render_template_string, make_response, send_from_directory, redirect, url_for
...
from CTFd.plugins import register_plugin_assets_directory, register_plugin_script, bypass_csrf_protection
...
SECRET_KEY = "ncu_mis_flag_salt_2026"
...
def generate_dynamic_flag(base_flag_text, team_name):
    ...
    seed_str = f"{SECRET_KEY}:{clean_team}:{inner}"
...
</html>''', login_url=login_url, register_url=register_url)
...
def download_campus():
    ...
    exif_dict = {
        ...
        "0th": {
            ...
            0x9c9f: "交接實驗室設備".encode("utf-16le"), # XPSubject -> Windows Details: 主旨
            ...
            0x9c9b: "學長逃跑現場".encode("utf-16le"), # XPTitle -> Windows Details: 標題
            ...
            piexif.ImageIFD.Artist: f"Senior in Kenting for {team_name}".encode("utf-8"),
...
def download_check_graduation_exe():
    ...
    if len(new_flag_bytes) < len(target_placeholder):
        ...
        new_flag_bytes = new_flag_bytes + bytes([0]) * (len(target_placeholder) - len(new_flag_bytes))
...
def lab_sqli():
    ...
    if request.method == "POST":
        ...
        sqli_patterns = ["' or 1=1", "' or '1'='1", "' or ''='", "' or true", "' or 1=1#", "' or 1=1--", "admin'--", "admin' #"]
        ...
        if any(p.replace(" ", "") in user_lower for p in sqli_patterns) or ("'" in username and ("or" in user_lower or "--" in username or "#" in username)):
...
def load(app):
    ...
    register_plugin_assets_directory(app, base_path="/plugins/dynamic_shuffle_flag/assets/")
